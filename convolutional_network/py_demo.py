...
def maxpool(X, f, s):
	...
	# s = int(round(s))
	(l, w, w) = X.shape
	pool = np.zeros((l, int(round((w-f)/s+1)),int(round((w-f)/s+1))))
	for jj in range(0,l):
		i=0
		while(i<w):
			j=0
			while(j<w):
				pool[jj,int(round(i/2)),int(round(j/2))] = np.max(X[jj,i:i+f,j:j+f])
				j+=s
			i+=s
	return pool
...
def ConvNet(image, label, filt1, filt2, bias1, bias2, theta3, bias3):
	...
	fc1 = pooled_layer.reshape(( int(round((w2/2)*(w2/2)*l2)),1))
	...
	dpool = dfc1.T.reshape((l2, int(round(w2/2)), int(round(w2/2))))
	...
	for jj in range(0,l2):
		...
		while(i<w2):
			...
			while(j<w2):
				...
				dconv2[jj,i+a,j+b] = dpool[jj,int(round(i/2)),int(round(j/2))]
				j+=2
			i+=2
...
def initialize_theta(NUM_OUTPUT, l_in):
	print(' NUM_OUTPUT Type :', type(NUM_OUTPUT), ' l_in :',type(int(round(l_in))))
	return 0.01*np.random.rand(NUM_OUTPUT, int(round(l_in)))
