...
class Model:
    ...
    def forward(self):
        with tf.variable_scope('user_id'):
            user_id_embed = tf.contrib.layers.embed_sequence(ids = self.placeholder['user_id'],vocab_size=paddle.dataset.movielens.max_user_id()+1
                ,embed_dim=32)
        ...
        with tf.variable_scope('age_id'):
            age_id_embed = tf.contrib.layers.embed_sequence(ids=self.placeholders['age_id'],vocab_size=len(paddle.dataset.movielens.age_table),
                embed_dim=16)
        ...
        with tf.variable_scope('movie_title'):
            movie_title_embed = tf.contrib.layers.embed_sequence(
                ids = self.placeholder['movie_title'],
                vocab_size = 5175,
                embed_dim=32
            )
    ...
    def backward(self):
        ...
        self.ops['loss'] = tf.reduce_mean(tf.squared_difference(self.ops['predict'],self.placeholders['score']))
        self.ops['train'] = tf.train.AdamOptimizer(self.lr).minimize(self.ops['loss'])


    def global_max_pooling(self,x):
        batch_size = tf.shape(self.placeholder['user_id'])[0]
        num_units = x.get_shape().as_list()[-1]
        x = tf.layers.max_pooling1d(x,x.get_shape().as_list()[1],1)
        x = tf.reshape(x,[batch_size,num_units])
        return x

    def train_batch(self,sess,user_id,gender_id,age_id,job_id,movie_id,category_ids,movie_title,score):
    ...
    def predict_batch(self,sess,user_id,gender_id,age_id,job_id,movie_id,category_ids,movie_title,score):
